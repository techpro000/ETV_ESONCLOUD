...
//            MyLog.video("======执行销毁进程===mediapLayer==removeCacheView");
//            if (mMediaPlayer != null) {
//                MyLog.video("======执行销毁进程===mediapLayer==removeCacheView test " + mMediaPlayer.isPlaying());
//                if (mMediaPlayer.isPlaying()) {
//                    mMediaPlayer.stop();
//                }
//                mMediaPlayer.release();
...
//        }
    ...
    public void clearMemory() {
        ...
        try {
            ...
            MyLog.video("======执行销毁进程===mediapLayer==clearMemory " + mMediaPlayer.isPlaying());
            mMediaPlayer.stop();
            mMediaPlayer.release();
            //20221223 去掉判断
            mMediaPlayer = null;
...
//            mMediaPlayer.release();
    ...
    private void playNextVideo() {
        try {
            ...
            MyLog.video("========== videoviewbitmap " + currentPlayIndex);
            MediAddEntity mediAddEntity = playList.get(currentPlayIndex);
            startToPlay(mediAddEntity);
        } catch (Exception e) {
