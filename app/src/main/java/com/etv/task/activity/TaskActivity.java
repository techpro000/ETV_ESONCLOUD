...
public abstract class TaskActivity extends BaseActivity {
    ...
    public void checkBackLightStatues(String printTag) {
        MyLog.playTask("===checkBackLightStatues===" + printTag);
        boolean isLightOpen = SystemManagerInstance.getInstance(TaskActivity.this).getBackLightTtatues("TaskPlayActivity onResume");
        MyLog.playTask("===checkBackLightStatues==当前背光状态=" + isLightOpen);
        if (!isLightOpen) {
            SystemManagerInstance.getInstance(TaskActivity.this).turnBackLightTtatues(true);
        }
    }
