...
//                    showModifyContentDialog(cpListEntity, textModify);
        ...
        try {
            ...
            switch (coActionType) {
                ...
                case AppInfo.TOUCH_TYPE_JUMP_SENCEN: //跳转场景
                    ...
                    MyLog.e("TOUCH", "=====跳转场景==coLinkAction==" + coLinkAction);
                    ...
                    //证明商标
                    MyLog.e("TOUCH", "=====跳转场景=000===" + nextSencenPosition + " /scenId= " + scenId);
    ...
    private void startTimerAutoJumpSencen() {
        MyLog.e("jumpTime", "========startTimerAutoJumpSencen=======");
        int sencentTime = getCurrentSencenPlayTime();
        if (sencentTime < 1) {
            return;
        }
        MyLog.e("jumpTime", "========sencentTime=======" + sencentTime);
        cacelTimerTask();
        timerCheckTask = new Timer(true);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case JUMP_SCENCE_BY_SCENCE_TIME:  //场景自带得时间
                    MyLog.e("jumpTime", "========JUMP_SCENCE_BY_SCENCE_TIME=======");  //场景根据播放时间得自动跳转
...
//            getPmFromTask(0, "第二个场景返回默认回到第一个场景");
    ...
    public void moveViewForward(boolean b) {
        MyLog.e("haha", "====列表的数量==moveViewForward==" + b);
        if (myPresentation != null) {
            myPresentation.moveViewForward(b);
        }
        if (view_abous == null) {
            MyLog.e("haha", "====列表的数量==view_abous == null==");
            return;
        }
        if (genratorViewList == null || genratorViewList.size() < 1) {
            MyLog.e("haha", "====列表的数量==0==");
            return;
        }
        MyLog.e("haha", "====列表的数量=====" + genratorViewList.size());
        for (int i = 0; i < genratorViewList.size(); i++) {
            Generator genView = genratorViewList.get(i).getGenerator();
