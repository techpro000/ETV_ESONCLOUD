    ...
    public static void writeFileToSystemApp(String filePath, String sysFilePath) {
        ...
        if (Build.VERSION.SDK_INT > 27 && Build.VERSION.SDK_INT < 29) {
            if (CpuModel.getMobileType().contains(CpuModel.CPU_MODEL_RK_3399)) {
                ...
                exusecmd("blockdev --setrw /dev/block/by-name/system", "安装守护进程9.0");
                exusecmd("mount -o rw,remount /", "安装守护进程9.0");
                exusecmd("rm -rf /system/app/guardian.apk", "安装守护进程9.0");
                exusecmd("cp  " + filePath + " " + sysFilePath, "安装守护进程9.0");
                exusecmd("chmod 777 /system/app/guardian.apk", "安装守护进程9.0");
                exusecmd("mount -o ro,remount /", "安装守护进程9.0");
            } else {
                //M8  9.0 这个方法不行，执行下面得
//                MyLog.guardian("====writeFileToSystemApp===9.0版本==3288=====");
//                exusecmd("mount -o remount -o rw /", "安装守护进程9.0");
//                exusecmd("rm -rf /system/app/guardian.apk", "安装守护进程9.0");
//                exusecmd("cp  " + filePath + " " + sysFilePath, "安装守护进程9.0");
//                exusecmd("chmod 644 /system/app/guardian.apk", "安装守护进程9.0");
//                exusecmd("sync", "安装守护进程9.0");
//                exusecmd("mount -o remount -o ro /", "安装守护进程9.0");
